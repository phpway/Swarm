...
$(function(){
    ...
    search.typeahead({
        source: function(query, process) {
            ...
            var escapedBaseUrl = ($('body').data('base-url') || '').replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g, '\\$&'),
                pathPattern    = new RegExp('^' + escapedBaseUrl + '\/files\/?'),
                queryString    = $.param({
                q:       query,
                max:     maxResults,
                path:    ($('.breadcrumb').data('path') || '').replace(pathPattern, '/'),
            ...
            delayTimeout = setTimeout(function(){
                ...
                var request, url = '/search?' + queryString + '&types=';
                $.each(['projects', 'groups', 'users', 'files-names', 'files-contents'], function(i, type){
    ...
    typeahead.select = function() {
        ...
        switch (item.type) {
            case 'user':
                ...
                break;
            case 'group':
                url = '/groups/' + swarm.encodeURIPath(item.id);
                break;
            case 'project':
