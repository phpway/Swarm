...
abstract class AbstractConnection implements ConnectionInterface
{
     ...
     *
     * @bool    $cache  optional - if true (default), then cached info (if available)
     *                  will be returned, otherwise return fresh info
     * @return  array   the connection information ('p4 info').
     */
    public function getInfo($cache = true)
    {
        // return cached info if available, unless $cache is false.
        if ($cache && isset($this->info) && $this->isConnected()) {
            return $this->info;
        }
     ...
     * @throws  P4\Exception    if the info doesn't contain 'serverDate' data
     */
    public function getServerTime()
    {
        $info = $this->getInfo(false);
        if (isset($info['serverDate'])) {
            return strtotime($info['serverDate']);
        }

        throw new P4\Exception("Cannot get server time; p4 info does not contain a serverDate data");
    }
