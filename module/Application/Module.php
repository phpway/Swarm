...
class Module
{
    ...
    public function onBootstrap(MvcEvent $event)
    {
        ...
        // in Multi-P4D mode, detect and handle routing requests that are lacking a Server ID
        $events->attach(
            ...
            function ($event) use ($config, $services) {
                ...
                // determine the first valid server and redirect the user to it
        ...
        // normalize base url if one is set.
        if (!empty($config['environment']['base_url'])) {
            $config['environment']['base_url'] = '/' . trim($config['environment']['base_url'], '/');
