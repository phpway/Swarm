...
class IndexController extends AbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        if (!strlen($topic) && $this->getRequest()->getQuery('format') !== 'json') {
        ...
        if ($this->getRequest()->getQuery('format') === 'json') {
            ...
            $comments   = Comment::fetchAll($this->getFetchAllOptions($topic, $query), $p4Admin, $ipProtects);
    ...
    public function addAction()
    {
        ...
        if ($request->getPost('bundleTopicComments', true)) {
            $data['comments'] = $isValid ? $comments($filter->getValue('topic')) : null;
        }
    ...
    public function editAction()
    {
        ...
        if ($request->getPost('bundleTopicComments', true)) {
            $data['comments'] = $comments($comment->get('topic'));
        }
